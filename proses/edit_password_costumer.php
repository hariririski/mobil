...
session_start(); // Memulai Session
	  include '../share/db.php';
			...
			$password_data_base;
			$username=$_SESSION['costumer'];
			$query = mysqli_query($con,"select * from costumer where username='$username' ");
			while($data = mysqli_fetch_array($query)){
					$password_data_base=$data['password'];
			}
			if($password_baru==$ulangi_password_baru){
				if($password_lama==$password_data_base){
					$perintah="update costumer set password='$password_baru' where username='$username'";
					...
					if ($query) {
								...
								echo '<script type="text/javascript">window.location = "../ubah_password_costumer.php?id='.$id.'"</script>';
					}
				}else{
					echo "<script>alert('Password Lama Tidak Cocok')</script>";
					echo '<script type="text/javascript">window.location = "../ubah_password_costumer.php"</script>';
				}
			}else{
